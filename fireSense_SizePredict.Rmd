...
# Overview
Predict the shape and taper parameters of the tapered Pareto from a model fitted using the fireSense_SizeFit module.
...
----|:-------|---------------------------------------------------------------------
`data`|`"dataFireSense_SizePredict"`|a character vector indicating the names of objects in the `simList` environment in which to look for variables in the model. `data` objects can be data.frames or named lists of RasterLayers, but data.frames and RasterLayers can not be mixed together. If omitted, or if variables are not found in `data` objects, variables are searched in the `simList` environment.
...
- **dataFireSense_SizePredict**: One or more data.frames, or named lists of RasterLayers in which to look for variables with which to predict.
