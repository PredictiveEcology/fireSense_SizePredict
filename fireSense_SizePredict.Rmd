...
----|:-------:|---------------------------------------------------------------------
`data`|`NULL`|optional. A character vector indicating the names of objects in the `simList` environment in which to look for variables in the model. Data objects can be `data.frame` or named `list` of `RasterLayer` but should all be of one unique class, e.g. `RasterLayer`. If omitted, or if variables are not found in data objects, variables are searched in the `simList` environment.
