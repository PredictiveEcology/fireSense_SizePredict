...
# Overview
Make predictions from a model fitted using fireSense_SizeFit.
...
----|:-------:|---------------------------------------------------------------------
data|NULL|optional. A character vector indicating the names of objects in the simList environment in which to look for variables in the model. Data objects can be data.frames or named lists of RasterLayers but should all be of one unique class, e.g. RasterLayer. If omitted, or if variables are not found in data objects, variables are searched in the simList environment.
...
## Make a typical output of fireSense_SizeFit
fireSense_SizeFitted <- list(
  formula = formula(beta = size_ha ~ weather2,
...
objects <- c("df", "fireSense_SizeFitted") # Pass objects found in the global environment to the simList environment.
paths <- list(
...
Predictions made using this module reflects the temporal and spatial heterogeneity found in environmental controls of fire spread. These can be used in simulation models drawing fire sizes from a tapered Pareto distribution, or to derive probability surfaces of spread probabilities (e.g. fireSense_SpreadPredict in association with fireSense_SpreadFit).
