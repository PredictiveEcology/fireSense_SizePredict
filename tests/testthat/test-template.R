...
#    `functionName` is a function's name in your module (e.g., `fireSense_SizePredictEvent1`).
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense_SizePredict/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_SizePredict")
  ...
  parameters <- list(
    ...
    fireSense_SizePredict = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_SizePredictEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_SizePredictEvent1(mySim)
  } else {
    simOutput <- mySim$fireSense_SizePredictEvent1(mySim)
  }
  ...
  if (exists("fireSense_SizePredictEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_SizePredictEvent2(mySim)
  } else {
    simOutput <- mySim$fireSense_SizePredictEvent2(mySim)
  }
